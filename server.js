...
//     console.error("No data to save for", filename);
//     return null;
//   }
//   const json2csvParser = new Parser();
//   const csv = json2csvParser.parse(data);
...
//         (symbol) => symbol.symbol
//       );
//       if (!validSymbols.includes(tradingPair)) {
//         console.error(`Invalid symbol: ${tradingPair}`);
//         return res
//           .status(400)
//           .json({ error: `Invalid symbol: ${tradingPair}` });
//       }
...
//           break;
//         case "orderbook":
//           url = `https://api.binance.us/api/v3/depth?symbol=${tradingPair}`;
//           filename = `${coin}_orderbook_data.csv`;
//           break;
//         case "tick":
//           url = `https://api.binance.us/api/v3/trades?symbol=${tradingPair}`;
...
//     console.log(`Received data: ${JSON.stringify(data)}`); // Log the API response

//     if (
//       !data ||
//       (Array.isArray(data) && data.length === 0) ||
//       (dataType === "orderbook" && (!data.asks || !data.bids))
//     ) {
//       console.error(`No data received for ${dataType} from ${exchange}`);
//       return res
//         .status(400)
//         .json({ error: `No data available for ${dataType}` });
//     }
